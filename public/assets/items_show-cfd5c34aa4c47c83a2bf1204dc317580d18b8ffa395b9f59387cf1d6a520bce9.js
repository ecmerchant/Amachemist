...
$("#hide").click(function () {
  if(document.getElementById("hide").innerText == "非表示"){
    ...
    document.getElementById("hide").className = "btn btn-info";
    handsontable.updateSettings({
      colWidths:colWidth,
      fixedColumnsLeft: 7
    });
...
$("#submit_url").click(function () {
  ...
  if(document.getElementById("submit_url").innerText == "アマゾン取得"){
    ...
    document.getElementById("submit_url").className = "btn btn-info";
...
$("#connet_yahoo").click(function () {
  ...
  if(document.getElementById("connet_yahoo").innerText == "ヤフオク取得"){
    ...
    document.getElementById("connet_yahoo").className = "btn btn-info";
    document.getElementById("progress").value = "cancel";
  }
});
...
$("#reload_yahoo").click(function () {
  ...
  if(document.getElementById("reload_yahoo").innerText == "データの更新"){
    ...
    document.getElementById("reload_yahoo").className = "btn btn-info";
    document.getElementById("progress").value = "cancel";
  }
});
...
$("#setcsv").click(function () {
  ...
  for(var i = 0; i < orgdata.length; i++){
    if(orgdata[i][0] == true){
      ...
      var point = Number(fixeddata[3][2]) / 100;
      var ampoint = Math.round(sellprice * point);
      var quantity = 1;
      var img = GetImage(sku);
      csvdata[3+j][0] = sku;
      ...
      csvdata[3+j][5] = "ASIN";
      //csvdata[3+j][21] = /"([\s\S]*?)"/.exec(orgdata[i][tcol+2])[1];
      for(var k = 0; k < 5; k++){
        csvdata[3+j][21+k] = img[k];
      }
      j++;
    }
  }
...
function GetImage(id){
  tempData = 'https://page.auctions.yahoo.co.jp/jp/auction/' + String(id);
  ...
  $.ajax({
    ...
    dataType: 'json',
    async: false,
    success: function (rmyData) {
      res = rmyData;
    },
    error: function (rmyData) {
      alert("NG");
    }
  });
  return res;
